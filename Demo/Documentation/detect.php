<?php
if(isset($_GET['detectuserLanguage']))
{
    require_once '../../core/init.php';
    if(isset($_POST['detect']) && $_POST['detect']  == "browser")
    {
        $html = _x("Your Browser Language is : ") . languageNameInEnglish(Detect::browser());
        $html .= "\n\n";
        $html .= _x("Your Browser Language is : ") . languageNameIntongue(Detect::browser());
...
<div class="container page-content" style="margin-top: 50px;">
    ...
    <div class="margin20 no-margin-left no-margin-right sub-header text-light">
        <h2> <?php __("Detect User Language") ?></h2>
        <br>
        <br>
 
    </div>
     <div class="example">
        <?php __("Logaty Library can detect your user language BY ( BROWSER LANGUAGE, COUNTRY LANGUAGE )!,") ?>
     </div>
    <h4><?php __("Part 1 : Browser Language :") ?></h4>
    <p>
        <?php __("Simle way to detect browser language for user") ?>
    </p>
...
echo Detect::browser();
        ...
        <p>
        <?php __("this method return ISO-2 code of Language") ?>
            <?php __("so you can do this :") ?>
        </p>
        <pre>
echo languageNameInEnglish(Detect::browser());
...
echo languageNameIntongue(Detect::browser());
        ...
        <button id="detect" class="button primary"><?php __("Try it Yourself") ?></button>
    </div>
...
<script>
    $(document).ready(function(){
        ...
        {
            $.ajax({
                url: "?detectuserLanguage=browser",
                type: 'POST',
                async: false,
                cache: false,
                data: {detect:"browser"},
